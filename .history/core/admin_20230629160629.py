...
class ColorPickerWidget(forms.TextInput):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        return mark_safe(
            ...
            f'    <input type="text" value="{color}" id="{input_id}"class="color-hex" name="{name}" style="width: 80px; margin-left: 5px;">'
            f'</div>'
            f'<script>'
            f'window.addEventListener("DOMContentLoaded", function() {{'
            f'    let coloret = document.getElementById("color-picker-{input_id}");'
            f'    let colordiv = coloret.parentNode;'
            f'    let inputElement = colordiv.querySelector("input");'
            f'    console.log(inputElement);'
            f'    inputElement.remove();'
            f'    var colorPicker = document.querySelector(".color-picker");'
            f'    var colorHex = document.querySelector(".color-hex");'
            f'    colorPicker.addEventListener("change", function() {{'
            f'        colorHex.value = colorPicker.value;'
            f'        inputElement.value = colorPicker.value;'
            f'    }});'
            f'    colorHex.addEventListener("input", function() {{'
            f'        colorPicker.value = colorHex.value;'
            f'        inputElement.value = colorHex.value;'
            f'    }});'
            f'    inputElement.addEventListener("input", function() {{'
            f'        colorPicker.value = inputElement.value;'
            f'        colorHex.value = inputElement.value;'
            f'    }});'
            f'}});'
            f'</script>'
        )
