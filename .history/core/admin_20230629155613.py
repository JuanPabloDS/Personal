...
class ColorPickerWidget(forms.TextInput):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        return mark_safe(
            ...
            f'    <input type="text" value="{color}" class="color-hex" name="{name}" style="width: 80px; margin-left: 5px;">'
            f'</div>'
            f'<script>'
            f'window.addEventListener("DOMContentLoaded", function() {{'
            f'    var coloret = document.getElementById("color-picker-{input_id}");'
            f'    var colordiv = coloret.parentNode;'
            f'    var inputElement = colordiv.querySelector("input");'
