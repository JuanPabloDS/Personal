...
class ColorPickerWidget(forms.TextInput):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        return mark_safe(
            ...
            f'    <input type="text" value="{color}" class="color-hex" id="{input_id}" style="width: 80px; margin-left: 5px;">'
            f'</div>'
            f'<script>'
            ...
            f'    var inputElem = document.querySelector("#color--{input_id}");'
            f'    var textElem = document.querySelector("#{input_id}");'
            f'    inputElem.addEventListener("change", function() {{'
            f'        textElem.value = inputElem.value;'
            f'    }});'
            f'    textElem.addEventListener("change", function() {{'
            f'        inputElem.value = textElem.value;'
            f'    }});'
            f'}});'
            f'</script>'
        )
