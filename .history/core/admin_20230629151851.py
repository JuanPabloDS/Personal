...
class ColorPickerWidget(forms.TextInput):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        return mark_safe(
            ...
            f'    <input type="color" value="{color}" class="color-picker" id="color-picker-{input_id}" style="margin-left: 5px;">'
            f'    <input type="text" value="{color}" class="color-hex" id="color-hex-{input_id}" name="{name}" style="width: 80px; margin-left: 5px;">'
            f'</div>'
            f'<script>'
            f'window.addEventListener("DOMContentLoaded", function() {{'
            f'    var inputElem = document.querySelector("#color-picker-{input_id}");'
            ...
            f'    }});'
            f'    textElem.addEventListener("input", function() {{'
