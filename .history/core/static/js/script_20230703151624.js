...
function toggleDiv(clickedDiv) {
  ...
  var fa = $(".question-box");
  var fa_caret = clickedContainer.find("i")
  fa_caret.removeClass("fa-caret-down");
  fa_caret.addClass("fa-caret-up");

  $(fa_caret).on("click", function() {
    $(this).siblings().removeClass("fa-caret-up");
  });


  // Fechar todas as outras divs
  allContainers.not(clickedContainer).find(".item").fadeOut();
  fa.not(fa_caret).find(".i").removeClass("fa-caret-up");
  fa.not(fa_caret).find(".i").addClass("fa-caret-down");
